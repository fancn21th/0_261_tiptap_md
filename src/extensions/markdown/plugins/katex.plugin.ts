...
// @ts-expect-error - markdown-it internal types
...
function isValidDelim(state: StateInline | StateBlock, pos: number) {
  let can_open = true,
    can_close = true;
...
const math_inline = (state: StateInline, silent: boolean) => {
  ...
  if (!silent) {
    ...
    token.markup = "$";
    token.attrs = [["latex", state.src.slice(start, match)]];
  }
...
const KatexPlugin = (md: MarkdownIt, options: KatexOptions) => {
  // Default options
